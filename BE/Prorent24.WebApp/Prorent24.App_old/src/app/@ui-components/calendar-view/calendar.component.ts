  ...
  _isInRange(date: Moment, month: Moment): boolean {
    ...
    if (this.IsRange && date.month() === month.month()) {
      ...
      if (ghostDate && this._hoveringSelectValue) {
        const start = moment
          .min(moment(ghostDate), moment(this._hoveringSelectValue))
          .toDate();
        const end = moment
          .max(moment(ghostDate), moment(this._hoveringSelectValue))
          .toDate();
  ...
  _buildYears(date: Moment): MonthInterface[][] {
    ...
    for (let i = 0; i < 12; i++) {
      months.push({
        ...
        year: date.year(),
        isCurrentMonth:
          moment(new Date()).month() === i && date.isSame(new Date(), "year"),
