  ...
  template: `
    <div class="wrap-scroller" #wrapper>
      <div class="horizontal-dates" style="position: relative;">
        <div
          class="d-flex flex-row full-width"
          #monthSlider
        ...
        >
          <ng-container *ngFor="let monthIndex of _List">
            <ng-container
              *ngIf="
                monthIndex === _selectedMonth || monthIndex == _showMonth;
                else determineBlock
              "
            >
              <a
                class="month-select selected-date"
                title=""
                #selectedElement
                (click)="_changeMonthView()"
              >
...
export class pgDateScroller implements OnInit {
  ...
  constructor(
    ...
    private picker: pgDatePickerComponent
  ) {
  ...
  _changeMonth(value: any) {
    if (this._selectedMonth > value) {
      this.picker._showMonth =
        this.picker._showMonth - (this._selectedMonth - value);
    } else {
      this.picker._showMonth =
        this.picker._showMonth + (value - this._selectedMonth);
    }
    this._selectedMonth = value;
  }
  ...
  alignToElement(): void {
    ...
    let offset = this._selectedElement
      ? this._selectedElement.nativeElement.offsetLeft + "px"
      : "0";
    let realOffset =
      -(
        parseFloat(offset) -
        (rect.width / 2 - this._selectedElement.nativeElement.offsetWidth / 2)
      ) + "px";
    this._renderer.setStyle(
      this._monthSlider.nativeElement,
      "transform",
