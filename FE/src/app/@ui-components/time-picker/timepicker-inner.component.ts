...
import { dropDownAnimation } from "@shared/animations/dropdown-animations";
                ...
                <li>
                  <span
                    ...
                    *ngIf="!(HideDisabledOptions && _hour.disabled)"
                    ...
                    (click)="
                      _selectHour(hourListInstance, _hour.index, _hour.disabled)
                    "
                  >
                    {{ _hour.name }}
                  </span>
              ...
              <ng-template ngFor let-_ampm [ngForOf]="_ampmList" let-i="index">
                <li
                  ...
                  *ngIf="!(HideDisabledOptions && _ampm.disabled)"
                  (click)="
                    _selectAMPM(ampmListInstance, _ampm.index, _ampm.disabled)
                  "
                >
                  {{ _ampm.name }}
                </li>
...
export class pgTimePickerInnerComponent
  ...
  _scrollToSelected(
    ...
    const currentOption = (instance.children[0].children[_transIndex] ||
  ...
  _selectAMPM(instance: HTMLElement, index: number, disabled: boolean): void {
    ...
    const tempDateString =
      ...
      "," +
      moment(this.Value).format(this._format) +
      " " +
      this._ampmList[index].name;
  ...
  _translateIndex(index: number, unit: string): number {
    ...
    if (unit === "hour") {
      ...
      const disabledMinutes =
        this.DisabledMinutes && this.DisabledMinutes(this._selectedHour);
      return this._calcIndex(disabledMinutes, index);
    } else if (unit === "second") {
      const disabledSeconds =
        this.DisabledSeconds &&
        this.DisabledSeconds(this._selectedHour, this._selectedMinute);
  ...
  _initPosition(): void {
    ...
    if (this._showHour) {
      this._scrollToSelected(
        this._hourListInstance.nativeElement,
        this._selectedHour,
        0,
        "hour"
      );
    ...
    if (this._showMinute) {
      this._scrollToSelected(
        this._minuteListInstance.nativeElement,
    ...
    if (this._showSecond) {
      this._scrollToSelected(
        this._secondListInstance.nativeElement,
        this._selectedSecond,
        0,
        "second"
      );
    }
    ...
    for (let i = 0; i <= 59; i++) {
      this._minuteList.push({
        disabled:
          ...
          this.DisabledMinutes(this._selectedHour).indexOf(i) !== -1,
