...
export class pgUploadComponent implements OnInit, OnChanges, OnDestroy {
  ...
  @Output() FileListChange: EventEmitter<UploadFile[]> = new EventEmitter<
    UploadFile[]
  >();
  ...
  @Output() Change: EventEmitter<UploadChangeParam> = new EventEmitter<
    UploadChangeParam
  >();
    ...
    if (this.Size > 0 && filters.findIndex(w => w.name === "size") === -1) {
      filters.push({
        ...
        fn: (fileList: UploadFile[]) =>
          fileList.filter(w => w.size / 1024 <= this.Size)
      });
    }
    if (
      this.FileType &&
  ...
  onRemove = (file: UploadFile): void => {
    ...
    file.status = "removed";
    ((this.Remove
      ? this.Remove instanceof Observable
        ? this.Remove
        : of(this.Remove(file))
  ...
  _setClassMap(): void {
    ...
    if (this.Type === "drag") {
      subCls = [
        this.FileList.some(file => file.status === "uploading") &&
          `${this._prefixCls}-drag-uploading`,
