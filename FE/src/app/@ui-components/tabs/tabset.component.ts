    ...
    >
      ...
      >
        <ng-template #tabBarExtraContent>
          <ng-template
            [ngTemplateOutlet]="TabBarExtraTemplate || TabBarExtraContent"
          ></ng-template>
        </ng-template>
        <li
          pg-tab-label
...
export class pgTabSetComponent
  ...
  private _createChangeEvent(index: number): TabChangeEvent {
    const event: TabChangeEvent = new TabChangeEvent();
    event.index = index;
  ...
  get inkBarAnimated(): boolean {
    return (
      this.Animated === true ||
      (this.Animated as AnimatedInterface).inkBar === true
    );
  }
  ...
  get tabPaneAnimated(): boolean {
    return (
      this.Animated === true ||
      (this.Animated as AnimatedInterface).tabPane === true
    );
  }
