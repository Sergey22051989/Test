...
import { measureScrollbar } from "@shared/utils/mesureScrollBar";
...
export class pgRangePickerComponent implements ControlValueAccessor, OnInit {
  ...
  _yearPanel: string[][] = [];
  _startDecade = new Array(2).fill(
    Math.floor(this._showYear[RangePart.Start] / 10) * 10
  );
  _triggerWidth = 0;
  ...
  @ViewChildren(pgTimePickerInnerComponent) timePickerInner: QueryList<
  ...
  _bindDisabledDateToPart(): void {
    ...
    this._disabledDatePart[RangePart.Start] =
      this._mode[RangePart.Start] === "month" ? null : this._disabledDate;
    this._disabledDatePart[RangePart.End] =
      this._mode[RangePart.End] === "month" ? null : this._disabledDate;
  }
  ...
  _initShow(): void {
    ...
    if (this.end.isValid() && !this.start.isSameOrAfter(this.end, "month")) {
      ...
      this._showMonth[RangePart.End] = this.start.isValid()
        ? nextMonthOfStart.month()
        : nextMonthOfNow.month();
      this._showYear[RangePart.End] = this.start.isValid()
        ? nextMonthOfStart.year()
        : nextMonthOfNow.year();
    }
  ...
  private _onChange(): void {
    if (
      this._isValid(RangePart.Start) &&
      this._isValid(RangePart.End) &&
      this.isValueChange()
    ) {
      this.onChange(this._value);
      this._oldValue = [...this._value];
